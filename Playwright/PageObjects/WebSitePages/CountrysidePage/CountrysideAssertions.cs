namespace PlaywrightRaffle.PageObjects
{
    public partial class Countryside
    {
        ...
        public static async Task VerifyTopBannerTitle()
        {
            ...
            Assert.That(text.ToLower(), Is.EqualTo(CountrysideTexts.TOP_TITLE.ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.TOP_TITLE}\" but was \"{text}\"");
        ...
        public static async Task VerifyTopBannerSubtitle()
        {
            ...
            Assert.That(text.ToLower(), Is.EqualTo(CountrysideTexts.TOP_SUBTITLE.ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.TOP_SUBTITLE}\" but was \"{text}\"");
        ...
        public static async Task VerifyBottomSliderTitle()
        {
            ...
            Assert.That(text.ToLower(), Is.EqualTo(CountrysideTexts.BOTTOM_SLIDER_TITLE.ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.BOTTOM_SLIDER_TITLE}\" but was \"{text}\"");
        ...
        public static async Task VerifyBottomSliderSubitle()
        {
            ...
            {
                Assert.That(listOfText[i].TextContentAsync().Result.ToLower(), Is.EqualTo(CountrysideTexts.BOTTOM_SLIDER_SUBTITLE[i].ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.BOTTOM_SLIDER_SUBTITLE[i]}\" but was \"{listOfText[i].TextContentAsync().Result}\"");
            }
        ...
        public static async Task VerifyInfoBlockTitles()
        {
            ...
            for (int i = 0; i < CountrysideTexts.TITLES_INFO_BLOCKS.Count; i++)
            {
                string expectedTitle = CountrysideTexts.TITLES_INFO_BLOCKS[i].ToLower();
        ...
        public static async Task VerifyInfoBlockParagraphs()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                Assert.That(listOfText[i].TextContentAsync().Result.ToLower(), Is.EqualTo(CountrysideTexts.PARAGRAPHS_INFO_BLOCKS[i].ToLower()), string.Concat("Not matched ", "\"", listOfText[i].TextContentAsync(), "\"", "\r\nwith ", "\"", CountrysideTexts.PARAGRAPHS_INFO_BLOCKS[i], "\""));
            }
        ...
        public static async Task VerifyHowItWorksTitle()
        {
            ...
            Assert.That(text.ToLower(), Is.EqualTo(CountrysideTexts.HOW_IT_WORKS_TITLE.ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.HOW_IT_WORKS_TITLE}\" but was \"{text}\"");
        ...
        public static async Task VerifyHowItWorksParagraph()
        {
            ...
            Assert.That(text.ToLower(), Is.EqualTo(CountrysideTexts.HOW_IT_WORKS_PARAGRAPH.ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.HOW_IT_WORKS_PARAGRAPH}\" but was \"{text}\"");
        ...
        public static async Task VerifyHowItWorksStepsTitles()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Assert.That(text[i].Result.ToLower(), Is.EqualTo(CountrysideTexts.TITLES_STEPS[i].ToLower()), $"Texts are not matched. Expected \"{CountrysideTexts.TITLES_STEPS[i]}\" but was \"{text[i].Result}\"");
            }
        ...
        public static async Task VerifyHowItWorksStepsParagraphs()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                string expectedParagraph = CountrysideTexts.PARAGRAPHS_STEPS[i].ToLower();
